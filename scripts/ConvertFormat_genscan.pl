...
for my $k1 (sort{$a <=> $b} keys %gff){
    ...
    for (my $i=0;$i<@cdss;$i++){
        ...
        my $j = $i+1;
        print "$chr\tgenscan\texon\t$cdss\t$cdse\t.\t$strand\t$codon\tID=genscan_g$k1.t1.exon$j;Parent=genscan_g$k1.t1\n";
    	print "$chr\tgenscan\tCDS\t$cdss\t$cdse\t.\t$strand\t$codon\tID=genscan_g$k1.t1.CDS$j;Parent=genscan_g$k1.t1\n";
	    $len += $cdse-$cdss+1;
    }
}
